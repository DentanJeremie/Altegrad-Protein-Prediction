...
from src.hgp.utils import load_data, normalize_adjacency
from src.hgp.models import Model, BaselineGATModel, GATModel
...
parser.add_argument('--weight_decay', type=float, default=0.0000001, help='weight decay')
...
parser.add_argument('--n_edge_features', type=int, default=1, help='number of edge features')
...
model = BaselineGATModel(args).to(device)
...
def train():
    ...
    plt.legend('lr : '+str(args.dropout_ratio))
    plt.show()
    return best_epoch
...
dropouts = [0,0.05,0.2,0.5,0.6,0.65,0.7]

if __name__ == '__main__':
    # Model training
    for dropout in dropouts :
        args.dropout_ratio = dropout
        best_model = train()
        # Restore best model for test set
        model.load_state_dict(torch.load('{}.pth'.format(best_model)))
        y_pred_proba = compute_test_final(test_loader)
        # Write predictions to a file
        with open('sample_submissionHGP14'+str(dropout)+'.csv', 'w') as csvfile:
